...
warnings.filterwarnings("ignore", category=FutureWarning, module="keras_ocr")
...
# extracting text from an image using Keras-OCR
def extract_text_from_image(image):
    image_array = np.array(image)
    # Convert image to grayscale for better OCR results
    image_gray = cv2.cvtColor(image_array, cv2.COLOR_RGB2GRAY)
    # Perform OCR
    prediction_groups = pipeline.recognize([image_gray])
    # Extract text
...
def calculate_pii_positions(image, pii):
    ...
    try:
        ...
        for box, word in prediction_groups[0]:
...
def detect_and_blur_pii(image):
    try:
        ...
        if pii:
            # pause and suggest detected pii to the user
...
def process_pdf(pdf_path):
    try:
        images = convert_from_path(pdf_path, poppler_path=r"C:\Users\yashj\Downloads\Release-24.07.0-0\poppler-24.07.0\Library\bin")
